...
import example.data.structure.model.LinkQueueNode;
...
public class LinkQueueDao {
     ...
     * 初始化队列
     */
    public LinkQueue initLinkQueue() {
        //初始化队列头结点
        LinkQueueNode head = new LinkQueueNode(0, null);
        //初始化队列
        return new LinkQueue(head, 0);
    }

    /**
     * 入队列
     */
    public void pushLinkQueue(LinkQueue queue, Object data) {
        if (queue == null) {
            return;
        }
        if (data == null) {
     ...
     * 出队列
     */
    public void popLinkQueue(LinkQueue queue) {
        if (queue == null) {
            return;
        }
        if (queue.size == 0) {
     ...
     * 返回队尾元素
     */
    public Object topLinkQueue(LinkQueue queue) {
        if (queue == null) {
            return null;
        }
        if (queue.size == 0) {
     ...
     * 返回队头元素
     */
    public Object frontLinkQueue(LinkQueue queue) {
        if (queue == null) {
            return null;
        }
        if (queue.size == 0) {
     ...
     * 返回队列容量，也就是队列元素个数
     */
    public int sizeLinkQueue(LinkQueue queue) {
        if (queue == null) {
            return -1;
        }
        return queue.size;
     ...
     * 判断队列是否为空
     */
    public boolean isEmptyLinkQueue(LinkQueue queue) {
        if (queue.size == 0) {
            return true;
        }
        return false;
     ...
     * 清空队列
     */
    public void clearLinkQueue(LinkQueue queue) {
        if (queue == null) {
            return;
        }
